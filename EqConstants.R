...
# check if salt content given in ionic strength or salinity
if (S_or_I > 1){
...
# KB used with H+(tot)
KB <- exp((- 8966.90 - 2890.53*S^0.5 - 77.942*S + 1.728*S^1.5 - 0.0996*S^2)/T +
...
KS <- exp(- 4276.1/T + 141.328 - 23.093*log(T) + (- 13856/T + 324.57 - 
             47.986*log(T))*I^0.5 + (35474/T - 771.54 + 114.723*log(T))*I - 
             2698/T*I^1.5 + 1776/T*I^2 + log(1-0.001005*S))

# KF used with H+(tot)
KF <- exp(874/T - 9.68 + 0.111*S^0.5)
...
KSi <-  exp(- 8904.2/T + 117.385 - 19.334*log(T) + (- 458.79/T + 3.5913)*I^0.5 +
             (188.74/T - 1.5998)*I + (- 12.1652/T + 0.07871)*I^2 + log(1 - 0.001005*S))

KP1 <-  exp(- 4576.752/T + 115.525 - 18.453*log(T) + (- 106.736/T + 0.69171)*S^0.5 +
             (- 0.65643/T - 0.01844)*S)

KP2 <-  exp(- 8814.715/T + 172.0883 - 27.927*log(T) + (- 160.340/T + 1.3566)*S^0.5 +
             (0.37335/T - 0.05778)*S)

KP3 <-  exp(- 3070.75/T - 18.141 + (17.27039/T + 2.81197)*S^0.5 + (- 44.99486/T - 0.09984)*S)
...
if (Sol == 1){
    ...
    KW      <- exp(- 13847.26/T + 148.9652 - 23.652*log(T) + (118.67/T - 5.977 + 
...
if (Sol == 1 || Sol == 3){
    m <- I*1000/(1000-I*(22.99+35.45))
        ...
        B2      <- -1738.16*m^0.5
        C2      <- -6.0346*m^0.5
        pK1     <- -402.56788 + 11656.46/T + 72.173*log(T) - 0.161325*T + 7.5526e-5*T^2
        pK2     <- -122.4994 + 5811.18/T + 20.5263*log(T) - 0.0120897*T
    #by necessity on H+(free)
    K1      <- 10^-( A1 + B1/T + C1*log(T) + pK1)
    #by necessity on H+(free)
    K2      <- 10^-( A2 + B2/T + C2*log(T) + pK2)
} else {
    #used with H+(tot)
...
return(list(k=k,KW=KW,K1=K1,K2=K2,ST=ST,FT=FT,BT=BT,KS=KS,KF=KF,KB=KB,KSi=KSi,KP1=KP1,KP2=KP2,KP3=KP3))
}

KW_in_NaCl <- function(c,t){
  # function to calculate KW in NaCl as a function of amount content of NaCl and temperature
       p00 <-       14.83
       p10 <-     -0.4914
       p01 <-     -0.0471
       p20 <-      0.3917
       ...
       p12 <-  -1.081e-05
       p03 <-  -4.382e-06
KW_NaCl <- 10^-(p00 + p10*c + p01*t + p20*c^2 + p11*c*t + p02*t^2 + p30*c^3 + p21*c^2*t +
                    p12*c*t^2 + p03*t^3)
return(KW_NaCl)
}

KW_in_KCl <-function(c,t){
  # function to calculate KW in KCl as a function of amount content of KCl and temperature
       p00 <-       14.86
       p10 <-      -1.062 
       p01 <-     -0.0479 
       p20 <-       2.031  
       ...
       p12 <-  -9.838e-06  
       p03 <-  -5.091e-06 
KW_KCl <- 10^-(p00 + p10*c + p01*t + p20*c^2 + p11*c*t + p02*t^2 + p30*c^3 + p21*c^2*t...
                    + p12*c*t^2 + p03*t^3)
return(KW_KCl)
}
