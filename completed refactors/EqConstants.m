...
function [k,KW,K1,K2,ST,FT,BT,KS,KF,KB,KSi,KP1,KP2,KP3] = EqConstants (S_or_I,t,Sol,BTrat)
...
if t  < 273.15 %check if temp in Kelvin or Celsius
    T  = mean(t)+273.15; t = mean(t);
else, T= mean(t); t = mean(t)-273.15; 
end
if S_or_I > 1
...
KB = exp((- 8966.90 - 2890.53.*S.^0.5 - 77.942.*S + 1.728.*S.^1.5 - 0.0996.*S.^2)./T ...
             ...
             0.2474.*S).*log(T) + 0.053105.*S.^0.5.*T); %used with H+(tot)
KS = exp(- 4276.1./T + 141.328 - 23.093.*log(T) + (- 13856./T + 324.57 - ...
             47.986.*log(T)).*I.^0.5 + (35474./T - 771.54 + 114.723.*log(T)).*I - ...
             2698./T.*I.^1.5 + 1776./T.*I.^2 + log(1-0.001005.*S)); %used with H+(free)
KF = exp(874./T - 9.68 + 0.111.*S.^0.5); %used with H+(tot)
...
elseif Sol == 3
    ...
    KW      = exp(- 13847.26./T + 148.9652 - 23.652.*log(T) + (118.67./T - 5.977 + ...
                1.0495.*log(T)).*S.^0.5 - 0.01615.*S); %used with H+(tot) --> not the original equation
end
...
if Sol == 1 || Sol == 3
    ...
    K1      = 10.^(- 3633.86./T + 61.2172 - 9.67770.*log(T) + 0.011555.*S - 0.0001152.*S.^2); %used with H+(tot)
    K2      = 10.^(- 471.78./T - 25.9290 + 3.16967.*log(T) + 0.01781.*S - 0.0001122.*S.^2); %used with H+(tot)
end
end
function KW_NaCl = KW_in_NaCl(c,t)
       p00 =       14.83;
       p10 =     -0.4914;
       p01 =     -0.0471;
       p20 =      0.3917;
